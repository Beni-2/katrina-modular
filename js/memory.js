...
async function loadServerMessages() {
  ...
  appendMsg('system', '--- thoughts while you were away ---');
  for (const m of unread) {
    const age     = (Date.now() - m.ts) / 1000;
    const timeStr = age < 60    ? 'just now'
      : age < 3600  ? Math.round(age / 60) + ' min ago'
      : age < 86400 ? Math.round(age / 3600) + 'h ago'
      : Math.round(age / 86400) + 'd ago';
    appendMsg('katrina', m.text);
    appendMsg('system', '(' + (m.type === 'reach-out' ? 'reached out' : 'thought to herself') + ' · ' + timeStr + ')');
    if (m.type === 'reach-out' && typeof speakText === 'function' && ttsEnabled) {
      setTimeout(() => speakText(m.text), 800);
    }
  }

  // Mark all as read
  const updated = messages.map(m => ({ ...m, read: true }));
  _sbSave('katrina_server_messages', updated);
}
